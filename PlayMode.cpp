...
GLuint interrogation_meshes_for_lit_color_texture_program = 0;
Load< MeshBuffer > interrogation_meshes(LoadTagDefault, []() -> MeshBuffer const * {
	MeshBuffer const *ret = new MeshBuffer(data_path("interrogation.pnct"));
	interrogation_meshes_for_lit_color_texture_program = ret->make_vao_for_program(lit_color_texture_program->program);
	return ret;
});

Load< Scene > interrogation_scene(LoadTagDefault, []() -> Scene const * {
	return new Scene(data_path("interrogation.scene"), [&](Scene &scene, Scene::Transform *transform, std::string const &mesh_name){
		Mesh const &mesh = interrogation_meshes->lookup(mesh_name);
		...
		drawable.pipeline.vao = interrogation_meshes_for_lit_color_texture_program;
...
PlayMode::PlayMode() : scene(*interrogation_scene) {
